...
public class SimpleInputFluidStore extends
    ...
    public int fill(FluidStack stack, FluidAction action) {
        ...
        for (int slot = 0; slot < tanks.size(); ++slot) {
            if (tanks.get(slot).getAmount() >= capacity) continue;
            if (tanks.get(slot).isEmpty() || FluidStack.isSameFluidSameComponents(tanks.get(slot), stack)) {
                final var amount = Math.min(tanks.get(slot).getAmount() + fluidLeft, capacity);
                fluidLeft -= amount - tanks.get(slot).getAmount();

                if (action.execute()) {
                    tanks.set(slot, stack.copyWithAmount(amount));
                    setChanged(slot);
                }
            }
